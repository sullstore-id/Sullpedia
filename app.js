...
setPersistence(auth, browserLocalPersistence).catch((error) => {
  console.error("Gagal set auth persistence:", error);
});
...
function finishSplash(callback) {
  ...
  const remainingTime = Math.max(minimumSplashTime - elapsed, 0);
...
const adminEwalletLink = document.getElementById("adminEwalletLink");
...
if (adminEwalletLink) {
  if (data.role === "admin") {
    adminEwalletLink.classList.remove("hidden");
  } else {
    adminEwalletLink.classList.add("hidden");
  }
}
  });
}
